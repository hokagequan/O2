...
- (void)scrollToTheSelectedCell {
    ...
    for (UITableViewCell *cell in [_contentTableView visibleCells]) {
        for (UILabel *label in [[cell subviews][0] subviews]) {
            label.textColor = [self.dataSource normalTextColorSelector:self];
        }
    }
    ...
    if (selectedIndexPath!=nil) {
        ...
        UITableViewCell *cell = [_contentTableView cellForRowAtIndexPath:selectedIndexPath];
        for (UILabel *label in [[cell subviews][0] subviews]) {
            label.textColor = [self.dataSource highlightTextColorSelector:self];
        }
