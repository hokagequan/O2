...
#define COLOR_NOAML ([UIColor colorWithRed:153/256.0  green:153/256.0 blue:153/256.0 alpha:1])
...
- (void)initCellView {
    ...
    //选中时显示的图片
    UIImageView *selectImageView = [[UIImageView alloc]initWithFrame:CGRectMake(0, 0, self.bounds.size.width, self.bounds.size.width)];
    selectImageView.image=[UIImage imageNamed:@"payment process@3x-16"];
...
- (void)setModel:(RMCalendarModel *)model {
    ...
    if (!model.ticketModel.ticketCount || model.style == CellDayTypePast) {
        ...
        self.price.textColor = [UIColor colorWithRed:102/255.0 green:102/255.0 blue:102/255.0 alpha:1];
        self.price.text = [NSString stringWithFormat:@"%.0f",model.ticketModel.ticketPrice];
    }
    ...
    switch (model.style) {
        ...
        case CellDayTypePast:
            ...
            self.dayLabel.textColor = [UIColor colorWithRed:153/255.0 green:153/255.0 blue:153/255.0 alpha:1];
            self.backgroundColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@"payment process@3x-17-17"]];
            break;
...
//            break;
        case CellDayTypeFutur:
            ...
            if (model.holiday) {
                ...
                self.dayLabel.width = self.bounds.size.width;
                self.chineseCalendar.hidden = YES;
                self.dayLabel.textColor = [UIColor colorWithRed:155/255.0 green:155/255.0 blue:155/255.0 alpha:1];
            } else {
                self.dayLabel.text = [NSString stringWithFormat:@"%lu",(unsigned long)model.day];
                self.dayLabel.textColor = [UIColor whiteColor];
            }
            self.chineseCalendar.textColor = [UIColor blackColor];
            self.selectImageView.image=[UIImage imageNamed:@"payment process@3x-15"];
            break;
        case CellDayTypeClick:
            self.dayLabel.hidden = NO;
            self.selectImageView.hidden = NO;
            self.selectImageView.image=[UIImage imageNamed:@"payment process@3x-16"];
            self.dayLabel.text = [NSString stringWithFormat:@"%lu",(unsigned long)model.day];
            self.dayLabel.textColor = [UIColor whiteColor];
