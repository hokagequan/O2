...
#define CATDayLabelHeight 21.0f

#define COLOR_THEME1 ([UIColor redColor])
#define COLOR_THEME ([UIColor colorWithRed:48/256.0  green:48/256.0 blue:48/256.0 alpha:1])
...
- (void)initWithHeader
{
    ...
    //月份
    UILabel *masterLabel = [[UILabel alloc] initWithFrame:CGRectMake(0.f, 0.f, headerWidth-32,24.f)];
    [masterLabel setTextAlignment:NSTextAlignmentCenter];
    [masterLabel setFont:[UIFont fontWithName:@"FZLanTingHeiS-EL-GB" size:13.0f]];
    self.masterLabel = masterLabel;
    self.masterLabel.textColor = COLOR_THEME;
    [self addSubview:self.masterLabel];
    CGFloat yOffset = 34.0f;
...
- (void)initHeaderWeekText:(NSString *)text titleColor:(UIColor *)color x:(CGFloat)x y:(CGFloat)y {
    UILabel *titleText = [[UILabel alloc]initWithFrame:CGRectMake(x, y, self.frame.size.width/7, CATDayLabelHeight)];
   
    [titleText setFont:[UIFont fontWithName:@"FZLanTingHeiS-EL-GB" size:10.0f]];
